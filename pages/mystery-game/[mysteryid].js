...
import styles from '@/styles/larp/mystery-info.module.css'
...
export default function MysteryId() {
  ...
  //跟伺服器抓資料
  const getEscape = async (mysteryid) => {
    const baseURL = `http://127.0.0.1:3006/mystery-game/${mysteryid}`
    if (!mysteryid) return // 檢查 larpid 是否已經存在，避免 undefined 錯誤
    try {
      const res = await fetch(baseURL)
      const resData = await res.json()
      // console.log('API Response:', resData)
      console.log('single Data:', resData.single)
  ...
  const getAllEscapes = async (mysteryid) => {
    const baseURL = `http://127.0.0.1:3006/mystery-game/${mysteryid}`
    const res = await fetch(baseURL)
    const resData = await res.json()
  ...
  useEffect(() => {
    if (router.isReady) {
      ...
      getEscape(router.query.mysteryid)
      getAllEscapes()
    }
  }, [router.isReady, router.query.mysteryid])
      ...
      <div className={styles.larpContainer}>
        <div className="position-relative d-flex justify-content-between">
          <div>
            {/* 劇本殺圖片 */}
            <Image
              src={`/larp/img/larp-product/${escape.larp_img}`}
              alt=""
              width={623}
              height={623}
            />
